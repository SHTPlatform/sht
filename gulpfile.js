...
function build(osVersion, netVersion) {
  //var dir = 'sht-' + osVersion + '-' + package.version + '-' + netVersion;
  var dir = 'sht-' + osVersion + '-latest-' + netVersion;
  var fullpath = path.join(__dirname, 'build', dir);
  var cmds = [];
  cmds.push(format('cd %s && mkdir -p public dapps tmp logs bin', fullpath));
  cmds.push(format('cp -r package.json shtd init proto %s', fullpath));
  if (netVersion != 'localnet') {
    cmds.push(format('sed -i "" "s/testnet/%s/g" %s/shtd', netVersion, fullpath));
...
function buildSource(netVersion) {
  // var dir = 'sht-' + 'linux' + '-' + package.version + '-' + netVersion;
  var dir = 'sht-' + 'linux' + '-latest-' + netVersion;
  var fullpath = path.join(__dirname, 'build', dir);
  return gulp.src('app.js')
    .pipe(webpack({
      output: {
